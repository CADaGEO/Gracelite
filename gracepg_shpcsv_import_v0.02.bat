...
REM http://postgis.17.x6.nabble.com/forced-conversion-of-linestring-to-mulitlinestring-when-using-shp2pgsql-utility-td3561122.html
...
ECHO Import %PGTBL%
REM shp2pgsql -s %PGSRID% -a -W %PGCODE% %PGSHP% %PGSCHEMA%.%PGTBL% > %PGTBL%.sql
shp2pgsql -t 2D -s %PGSRID% -a -W %PGCODE% %PGSHP% %PGSCHEMA%.%PGTBL% > %PGTBL%.sql
REM psql -h localhost -p 5433 -d gracethd-v2-beta1a -U postgres -f t_adresse.sql
psql -h %PGHOSTNAME% -p %PGPORT% -d %PGDB% -U %PGUSER% -f %PGTBL%.sql
...
psql -h %PGHOSTNAME% -p %PGPORT% -d %PGDB% -U %PGUSER% -c "COPY %PGSCHEMA%.%PGTBL% FROM '%PGCSV%' %PGCSVCONF%;"
...
ECHO Import %PGTBL%
REM shp2pgsql -s %PGSRID% -a -W %PGCODE% %PGSHP% %PGSCHEMA%.%PGTBL% > %PGTBL%.sql
shp2pgsql -t 2D -S -s %PGSRID% -a -W %PGCODE% %PGSHP% %PGSCHEMA%.%PGTBL% > %PGTBL%.sql
REM psql -h localhost -p 5433 -d gracethd-v2-beta1a -U postgres -f t_adresse.sql
psql -h %PGHOSTNAME% -p %PGPORT% -d %PGDB% -U %PGUSER% -f %PGTBL%.sql
